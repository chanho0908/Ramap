...
| 작업 유형 | 승인 필요 여부 | 설명 |
|----------|--------------|------|
| 계획 수립 | ✅ 필요 | 구현 작업은 planner가 계획을 먼저 작성 |
| 코드 구현 | ✅ 필요 | 사용자 계획 승인 후 implementer가 실행 |
...
구현 요청이 명확하더라도 계획 승인 없이 코드 작성 단계로 넘어가지 않습니다.
...
- 구현 작업에는 작은 작업 예외를 적용하지 않습니다.
- 오타 수정, 문서 업데이트처럼 코드 동작을 바꾸지 않는 작업은 planner 생략 가능
- 이 경우에도 develop/main에서 직접 작업하지 않고 작업 브랜치를 사용합니다.
